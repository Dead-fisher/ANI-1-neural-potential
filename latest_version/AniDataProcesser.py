...
class AEV_transformer(object):
    def __init__(self, max_atoms=23, radial_cutoff=4.6, angular_cutoff=3.1, radial_length=32,
                    angular_length=8, atom_cases=[1, 6, 7, 8], atomic_number_differentiated=True,
    ...
    def distance_matrix(self, coordinates):
        ...
        return torch.sqrt((-2*coordinates.mm(coordinates.t()))+\
                          ...
                          torch.sum(torch.square(coordinates.t()),axis=0,keepdim=True)+1e-5)
    ...
    def angular_symmetry(self, d_cutoff, d, atomic_numbers, coordinates):
        ...
        Rs = torch.linspace(0, self.angular_cutoff, self.angular_length)
        eta = float(3 / (Rs[1] - Rs[0])**2)
        thetas = torch.linspace(0, np.pi, self.angular_length)
        zeta = float(self.angular_length**2)

        Rs, thetas = torch.meshgrid(Rs, thetas)
        Rs = Rs.view(Rs.shape[0], Rs.shape[1], 1, 1, 1)
        thetas = thetas.view(thetas.shape[0], thetas.shape[1], 1, 1, 1)
        ...
        if (self.atomic_number_differentiated):
            ...
            for iter_i in range(iter_size):
                for iter_j in range(iter_i, iter_size):
                    ...
                    out_tensors.append(torch.sum((out*mask), axis=[3,4]))
            return torch.transpose(torch.stack(out_tensors).view(-1, tot_atom_numbers), 1, 0)
        
        else:
            return torch.transpose(torch.sum(out, axis=[3,4]).view(-1, tot_atom_numbers), 1, 0)
    
    def calculate_AEV(self, input_crd, input_atomic_number):
        '''
        ...
        angular_symmetry_function = self.angular_symmetry(distance_matrix_cutoff, distance_matrix, input_atomic_number, input_crd)
        return torch.cat([radial_symmetry_function, angular_symmetry_function], axis=1)

    def __call__(self, data, train_test_ratio=0.8):
        '''
        Create trainable torch Tensor from molecular data read from .h5 file

        Args:
            data: molecular information read by ani_data_loader
            train_test_ratio: ratio of training set

        Returns:
            train_AEV, test_AEV, train_ene, test_ene in torch.tensor form 
            train_AEV: [train size, atom numbers, AEV vector size]
            test_AEV: [test size, atom numbers, AEV vector size]
            train_ene: [train size, 1]
            test_ene: [test size, 1]
        '''
        ...
        train_size = train_crd.shape[0]
        test_size = test_crd.shape[0]
        
        atomic_number = torch.tensor([self.atomic_number_dic[i] for i in species])
        
        train_AEV = [self.calculate_AEV(train_crd[i], atomic_number) for i in range(train_size)]
        train_AEV = torch.stack(train_AEV)

        test_AEV = [self.calculate_AEV(test_crd[i], atomic_number) for i in range(test_size)]
        test_AEV = torch.stack(test_AEV)

        return train_AEV, test_AEV, train_ene, test_ene