...
class MoleculeANI(nn.Module):
    ...
    def forward(self, data, species):
        ...
        output = torch.zeros((1, data.shape[0]), device=data.device).flatten()
    ...
    def save_model(self, optimizer, epoch, loss_name):
        state = {
            'net': self.state_dict(),
        ...
        if not os.path.isdir('./models'):
            os.mkdir('./models')
        torch.save(state, './models/ani_para_' + str(epoch) + str(loss_name) + '.pth')
        return
...
def write_log(file, epoch, batch, loss, loss_name):
    '''
    Write logging while training.
    Args:
        file: file path to write.
        epoch, batch, loss, loss_name: information in files.
    Returns:
         None.
    '''
    ...
    with open('./logs/'+file, 'a') as f:
        f.write("{},{},{},{}\n".format(epoch, batch, loss, loss_name))


def init_log(file):
    '''
     To initialize logging file before writing, mainly to clear contains.
     Args:
         file: file path to write.
     Returns:
          None.
